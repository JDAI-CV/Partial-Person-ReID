...
class TestSet(Dataset):
  ...
  def extract_feat(self, normalize_feat, verbose=True):
    ...
    """
    feat, spatial_feat, ids, cams, im_names, marks = [], [], [], [], [], []
    done = False
    step = 0
    printed = False
    st = time.time()
    ...
    while not done:
      ...
      feat_, feat1_ = self.extract_feat_func(ims_)
      feat.append(feat_)
      ...
      if verbose:
        ...
        if step % 20 == 0:
          ...
          last_time = time.time()
    feat = np.vstack(feat)
    spatial_feat = np.vstack(spatial_feat)
    ids = np.hstack(ids)
    cams = np.hstack(cams)
    ...
    return feat, spatial_feat, ids, cams, im_names, marks
  ...
  def eval(
    ...
    with measure_time('Extracting feature...', verbose=verbose):
      feat, spatial_feat, ids, cams, im_names, marks = self.extract_feat(
    ...
    with measure_time('Computing distance...', verbose=verbose):
      ...
      for lam in range(0, 11):
        ...
        with measure_time('Computing scores...', verbose=verbose):
          q_g_dist = (1-weight) * q_g_global_dist + weight * q_g_spatial_dist
    ...
    if any(mq_inds):
      ...
      mq_spatial_feat = spatial_feat[mq_inds]
      ...
      mq_feat = np.stack([pool(mq_feat[unique_mq_ids_cams[k]], axis=0)
                          for k in keys])
      mq_spatial_feat = np.stack([pool(mq_spatial_feat[unique_mq_ids_cams[k]], axis=0)
                          for k in keys])
      ...
      for lam in range(0,11):
        weight = 0.1*lam
        with measure_time('Computing scores...', verbose=verbose):
          mq_g_dist = (1-weight)*mq_g_global_dist + weight*mq_g_spatial_dist
          mq_mAP, mq_cmc_scores = compute_score(mq_g_dist, query_ids=np.array(zip(*keys)[0]), gallery_ids=ids[g_inds], query_cams=np.array(zip(*keys)[1]), gallery_cams=cams[g_inds])
