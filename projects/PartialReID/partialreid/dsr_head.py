...
class OcclusionUnit(nn.Module):
    ...
    def forward(self, x):
       ...
       # mask_weight_norm = torch.sigmoid(mask_weight)
        ...
        global_feats = mask_score.matmul(SpaFeat1).view(SpaFeat1.shape[0], -1)
...
class DSRHead(nn.Module):
    def __init__(self, cfg, in_feat, num_classes, pool_layer=nn.AdaptiveAvgPool2d(1)):
        ...
        self.bnneck_occ = nn.BatchNorm1d(in_feat)
        self.bnneck_occ.bias.requires_grad_(False)
    ...
    def forward(self, features, targets=None):
       ...
       # pdb.set_trace()
        return pred_class_logits, global_feat, fore_pred_class_legits, foreground_feat, targets
